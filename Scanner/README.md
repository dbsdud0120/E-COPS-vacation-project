...
SQLi, XSS, Stored XSS, Directory Traversal, File Upload, IDOR, Security Header,
JWT 검증 누락, Broken Authentication, Rate Limiting 등 총 10개 검사를 지원합니다.

## 검사 목록

실제 Backend 컨테이너를 대상으로 아래 10개 검사를 전부 실행해서, 의도한 취약 라우트에서 정확히 탐지되는 것까지 확인했습니다.

| 검사 이름 (check_name) | 무엇을 찾는지 | 필요한 payload 파일 |
|---|---|---|
| `sql_injection` | GET 쿼리 파라미터 + POST 폼 필드에 SQL 페이로드를 전송해 DB 에러 시그니처가 응답에 노출되는지 확인 | `sql_injection.txt` (SQL 인젝션 문자열 목록) |
| `xss` | GET 쿼리 파라미터 + POST 폼 필드에 스크립트 페이로드를 전송해 이스케이프 없이 그대로 반사되는지 확인 (반사형 XSS) | `xss.txt` (XSS 페이로드 목록) |
| `stored_xss` | POST 폼에 고유 marker가 담긴 스크립트를 제출한 뒤, 페이지를 다시 조회해서 이스케이프 없이 저장·노출되는지 확인 (저장형 XSS) | 사용 안 함 — 실행마다 고유 payload를 코드에서 직접 생성 |
| `directory_traversal` | 파일 경로 파라미터에 `../` 같은 경로 조작 페이로드를 넣어 상위 디렉터리 파일 접근이 가능한지 확인 | `directory_traversal.txt` |
| `file_upload` | 위험한 확장자(.php, .jsp 등) 파일을 업로드해서 검증 없이 수락되는지 확인 | `file_upload.txt` (위험 파일명 목록) |
| `idor` | 서로 다른 계정 2개로 로그인해서, 한 계정이 다른 계정 소유 리소스(글 수정/삭제 등)에 접근 가능한지 확인 | `idor.txt` (계정 2개, `아이디:비밀번호` 형식) |
| `missing_jwt_verification` | 응답에서 발견한 JWT를 변조(alg=none, 서명 조작)한 뒤 쿠키 없이 요청해서 서버가 서명 검증 없이 그대로 받아주는지 확인 | 사용 안 함 — 토큰을 응답에서 직접 탐지 (스캔 시작 시 `/api/token`으로 미리 발급받아 확보) |
| `broken_authentication` | 세션 쿠키 보안 속성(HttpOnly/Secure/SameSite), 세션 토큰 재사용 여부, 로그인 실패 횟수 제한(계정 잠금) 여부 확인 | `broken_authentication.txt` (계정 1개, `아이디:비밀번호` 형식) |
| `missing_rate_limiting` | 로그인/가입 등 민감한 엔드포인트에 짧은 시간 연속 요청을 보내 429/503 같은 제한이 걸리는지 확인 | 사용 안 함 — 더미 요청을 코드에서 직접 생성 |
| `security_headers` | 응답 헤더에 CSP, X-Frame-Options, X-Content-Type-Options 등 보안 헤더가 있는지 확인 | 사용 안 함 — 필수 헤더 목록을 코드에서 직접 관리 |
...
│   ├── missing_jwt_verification.txt  # 빈 파일 (payload 불필요, 응답에서 토큰을 직접 찾음)
...
| `checks/idor.py` | ID 기반 URL에 대해 인증 없이/계정 간 접근 가능 여부 확인. 응답 JSON에 소유자 필드(`writer` 등)가 있으면 실제로 타 계정 소유 데이터에 접근했는지까지 구분해서 기록. 삭제형 URL은 실제 요청을 보내지 않음(안전장치) |
| `checks/security_headers.py` | 응답 헤더에 CSP, X-Frame-Options 등 보안 헤더가 빠졌는지 확인 |
| `checks/file_upload.py` | `input type="file"`이 있는 POST 폼에 위험한 확장자(.php, .jsp 등) 파일을 실제 업로드해서 검증 없이 수락되는지 확인 |
| `checks/jwt_verification.py` | 응답(쿠키/바디/헤더)에서 JWT를 찾아 alg=none·서명 변조 토큰으로 재요청, 서명 검증 없이 통과되는지 확인 |
| `checks/broken_authentication.py` | 로그인 페이지 대상, 세션 쿠키 보안 속성·토큰 예측 가능성·로그인 실패 횟수 제한(계정 잠금) 여부 확인 |
| `checks/rate_limiting.py` | 로그인 등 민감 엔드포인트에 연속 요청을 보내 429/Retry-After 등 제한이 걸리는지 확인 |
...
## Backend 쪽 실제 구조에 맞춰 넣은 설정값 (바뀌면 여기만 수정)
...
| `checks/base.py` | `VULN_TYPE_MAP` | 아래 참고 | check_name → Report 표기 이름. Report의 `mitigation_guide.md` 표와 정확히 같은 문자열이어야 함 |
...
Report(`report_generator.py`)가 `target`, `scan_date`, `vulnerabilities`(각 항목의
...
- **Scanner ↔ Report 스키마 정렬**: `findings` → `vulnerabilities`, `check_name` → `type`(`VULN_TYPE_MAP`으로
  `mitigation_guide.md`와 동일한 문자열 매핑), severity Capitalize(`SEVERITY_DISPLAY_MAP`, "Info" 등급 포함)까지
  모두 `Finding.to_dict()`/`scanner.py` 출력 시점에 처리되어 각 `checks/*.py` 코드는 내부 값만 신경 쓰면 됨.
- **IDOR 소유권 비교**: `checks/idor.py`는 응답 JSON의 소유자 필드(`OWNER_FIELD_CANDIDATES`, 예: `writer`)를
  비교해 "권한 검증이 아예 없다"뿐 아니라 "타 계정 소유 데이터를 실제로 열람했다"까지 구분해서 evidence에 기록함.
  소유자 필드를 못 찾는 경우(응답이 JSON이 아니거나 후보 필드가 전혀 없는 경우)에는 "권한 검증 없음으로 추정"만
  기록해 오탐(false positive)을 만들지 않음.

## 알려진 제약 사항

- `mitigation_guide.md`에 `security_headers` 검사에 대응하는 "Security Headers" 행이 아직 없어,
  리포트에는 해당 항목이 "대응 방안 미정의"로 표시됨 (Report 쪽에서 행 추가 필요)
- swagger.yaml이 정적 파일로만 존재하고 서버 라우트로는 노출되지 않아, 현재는 로컬 파일 경로로 `--swagger`
  값을 넘겨서 사용 중. 나중에 `/swagger.json` 같은 라우트로 노출되면 URL로 바꾸기만 하면 됨
- `/api/token`은 GET이 없는 POST 전용 라우트라 크롤러가 스스로는 찾지 못하므로, 스캔 시작 시 로그인 계정으로
  미리 토큰을 발급받아 확보해두는 방식으로 동작함 (`scanner.py` 참고)
